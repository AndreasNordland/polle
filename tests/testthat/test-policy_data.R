...
# test_that("set_variable_to_default sets the variable to the default while checking for duplicates.", {
#   data <- data.table(id = 1:10, ID = 10:1)
#   id <- "ID"
#   data <- set_variable_to_default(data = data, variable = id, default = "id")
#   expect_equal(data, data.table(id.1 = 1:10, id = 10:1))
#   rm(data, id)
#
#   data <- data.table(id = 1:10, ID = 10:1)
#   id <- "id"
#   data <- set_variable_to_default(data = data, variable = id, default = "id")
#   expect_equal(data, data.table(id = 1:10, ID = 10:1))
#   rm(data, id)
#
#   data <- data.table(id = 1:10, ID = 10:1)
#   data <- set_variable_to_default(data = data, default = "id")
#   expect_equal(data, data.table(id = 1:10, ID = 10:1))
#   rm(data)
#
#   data <- data.table(id = 1:10, ID = 10:1)
#   data <- set_variable_to_default(data = data, default = "id")
#   expect_equal(data, data.table(id = 1:10, varible = NULL, ID = 10:1))
#   rm(data)
#
#   data <- data.table(V1 = 1:10, V2 = 10:1)
#   var <- c("A", "B")
#   expect_error(set_variable_to_default(data = data, variable = var, default = "id"), "'var' must be a character string.")
#   rm(data)
#
#   data <- data.table(ID2 = 1:10, ID = 10:1)
#   id <- "id"
#   expect_error(set_variable_to_default(data = data, variable = id, default = "id"), "'id' is invalid.")
# })


# test_that("policy_data receives valid data",{
#   expect_error(policy_data(data = c("A", "B")), "'data' must be a data.table.")
#   expect_error(policy_data(data = data.table(id = 1:2), baseline_data = c("A", "B")), "'baseline_data' must be a data.table.")
#
#   data <- data.table(id = 1:2, action = c(1,2))
#   policy_data(data = data)
#
#
#   data <- data.table(id = 1:n, id = 10:1)
#   expect_error(policy_data(data = data, type = "wide"), "'data' has duplicated variable names.")
#   data <- data.table(X = 1:n, X = 10:1)
#   expect_error(policy_data(data = data, type = "wide"), "'data' has duplicated variable names.")
# })
