...
#' \deqn{
#' z, x \sim \text{Uniform}([0,1])\\
#' \tilde a \sim \mathcal{N}(0,1)\\
#' a \mid \tilde a \sim
#' \begin{cases}
#' 0 & \text{if} \quad \tilde a < -1\\
#' 1 & \text{if} \quad \tilde a -1 \leq a < 0.5\\
#' 2 & \text{otherwise}
#' \end{cases}\\
...
#' L_{1} \sim \mathcal{N}(0, 1)\\
#' C_{1} \mid L_{1} \sim \mathcal{N}(L_1, 1)\\
#' \mathbb{P}(A_1='yes'\mid C_1) =  \text{expit}(\beta C_1)\\
#' \mathbb{P}(A_1='no'\mid C_1) = 1 - \mathbb{P}(A_1='yes' \mid C_1)\\
#' L_{2} \sim \mathcal{N} (0, 1)\\
#' C_{2} \mid A_1, L_1 \sim \mathcal{N}(\gamma L_1 + A_1, 1)\\
#' \mathbb{P}(A_2='yes') = p_1\\
#' \mathbb{P}(A_2='no') = p_2\\
#' \mathbb{P}(A_2='default') = p_3\\
#' L_{3} \sim \mathcal{N} (0, 1)
#' }
#' The rewards are calculated as
#' \deqn{
...
#' \begin{cases}
#' \text{Exp}\Big\{\exp\left(\gamma^T [1, X_{k-1}, W] \right) \Big\} + \psi & A_{k-1} = 1\\
#' \infty & A_{k-1} = 0
#' \end{cases}\\
#' X_{k}\mid T_k, X_{k-1}, B \sim
#' \begin{cases}
#' \text{N}\left\{ \alpha^T [1, T_k, T^2_k, X_{k-1}, B], 1\right\} & T_k < \infty \\
#' 0 & T_k = \infty
#' \end{cases}\\
#' A_k \mid X_k, T_k \sim
#' \begin{cases}
#' \text{Ber}\left\{ \text{expit}\left(\beta^T[1, T_{k}^2, X_k] \right)\right\} & T_k < \infty\\
#' 0 & T_k = \infty,
#' \end{cases}
#' }
