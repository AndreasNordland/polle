...
As in the associated package manuscript, terminal events may occur.

![Figure caption](figures/right_cens_figure.svg)


To deal with this, the methodology for a fixed stage problem can be applied to auxiliary data.

For example, if a terminal event occurs at stage 2 we observe

\begin{align*}
O^\ast = \{B, X_1, U_1, \Delta_1, \Delta_1 A_1, \Delta_1 X_2, \Delta_1 U_2\}.
\end{align*}

Note that we here explicitly write $U_1$ and $U_2$. Remember that in full generality $U = U_1 + U_2$ if we observe a terminal event at stage $2$.

We then construct auxiliary data

\begin{align*}
\{\Delta_2 = 0,  A_2 = a^\dagger, X_3 = \emptyset,  \Delta_3 = 0,  U_3 = 0\}
\end{align*}

and calculate the (un-centralized) doubly robust score using that if a terminal event happens at stage 2

\begin{align*}
&C_2(H_2) = C_3(H_3) = 1\\
&Q_2^d(H_2, a^\dagger) = Q_3^d(H_3) = Q_4^d(H_4) = U_1 + U_2 = U
\end{align*}
