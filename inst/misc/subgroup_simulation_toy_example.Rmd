---
title: "Subgroup analysis toy simulation"
output:
...
```{r}
## fitting the policy by creating a policy object:
po1 <- pl1(
    policy_data = pd,
    g_models = g_glm(~1),
    q_models = q_glm(~ A * (W + L))
)
## getting the policy function:
pf1 <- get_policy(po1)
## policy actions
pa <- pf1(pd)
```
...
## Simulation - no cross-fitting

```{r onerun}
onerun <- function(n, par, n_approx, policy_learn) {
  ...
  ## subgroup average treatment effect estimate:
  pe <- policy_eval(
    policy_data = pd,
  ...
  ## subgroup average treatment effect (for the fitted policy):
  po <- get_policy_object(pe)
  pf <- get_policy_functions(po, stage = 1)

  approx <- sim_d(n = n_approx, par = par, potential_outcomes = TRUE, action_function = pf)
  target <- with(approx, mean((U1 - U0)[(A == 1)]))
...
```{r}
## single run:
onerun(n = 200, par = par0, n_approx = 1e7, policy_learn = pl1)
```

```{r sim_asymp, cache = TRUE}
plan("multisession")
res <- sim(
  x = onerun,
  R = 1e3,
...
```{r}
summary_adaptive <- function(m){
  est <- as.vector(m[, "est"])
  se <- as.vector(m[, "se"])
  target <- as.vector(m[, "target"])
  ...
  indicator <- (target <= upper) & (target >= lower)

  coverage <- mean(indicator)
